    ...
    public class CloudBlobClient : IFileUploader
    {
        ...
        public CloudBlobClient(Uri uploadUrl, UserFile file)
        {
            if (file != null)
            {
                ...
                this.file = file;
                this.packets = this.packager.TransformStreamToPackets(file.FileStream);
            }
        }
        ...
        /// Starts the upload.
        /// </summary>
        /// <param name="initParams">The init parameters.</param>
        ...
        private void NotifyClient(Constants.UploadCompleteReason haltReason, string errorMessage = "")
        {
        ...
        private void UploadFileChunk(DataPacket requestPayload, UserFile fileToUpload, Uri blockUri = null)
        {
            if (!this.cancellationTokens.IsCancellationRequested)
            {
                if (requestPayload != null && fileToUpload != null)
                {
                    ...
                    webRequest.Method = Constants.RequestType.PUT.ToString();
        ...
        private void WriteToStreamCallback(IAsyncResult asynchronousResult)
        {
            if (!this.cancellationTokens.IsCancellationRequested)
            {
                ...
                requestStream.Write(requestState.RequestPayload.Payload, 0, requestState.RequestPayload.Payload.Length);
                requestStream.Close();
                var webRequestState = new AsyncWebRequestState()
                    {
                        WebRequestState = webRequest,
        ...
        private void ReadHttpResponseCallback(IAsyncResult asynchronousResult)
        {
            if (!this.cancellationTokens.IsCancellationRequested)
            {
                ...
                lock (this.lockingObject)
                {
                    ...
                    {
                        this.NotifyClient(Constants.UploadCompleteReason.UploadCommitted);
                    }
                }
            }
        }
        ...
        private void PutBlockList(AsyncWebRequestState requestState)
        {
            if (!this.cancellationTokens.IsCancellationRequested)
            {
                ...
                webRequest.Method = Constants.RequestType.PUT.ToString();
                webRequest.Headers[MsVersionHeader] = Constants.XMSVersion;
        ...
        private void BlockListWriteToStreamCallback(IAsyncResult asynchronousResult)
        {
            if (!this.cancellationTokens.IsCancellationRequested)
            {
                ...
                this.request.UncommittedBlockBlobList(this.packets.Count()).Save(writer);
                writer.Flush();
                requestStream.Close();
                requestState.WebRequestState = webRequest;
